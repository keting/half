...
1. 填写项目名称、项目目标。
2. 填写 HALF 协作仓库地址和协作目录（目录留空可走默认目录策略）。单仓库工作流保持项目代码仓库与协作仓库相同；代码改动需要提交到另一个仓库时，再单独填写项目代码仓库地址。若使用私有仓库，请先按 `quickstart.zh-CN.md` 完成 GitHub 访问配置（SSH key 或令牌）。
...
1. 进入编辑页后，按需修改项目名称、项目目标、HALF 协作仓库地址、项目代码仓库地址、协作目录。
