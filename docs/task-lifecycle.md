...
| **需人工处理** | 超过任务超时时间（`tasks.timeout_minutes`）仍未检测到 `result.json`。git sync 失败不会转入此状态，任务会保持 `running` 并把错误信息写入 `last_error`；`result.json` 是否格式合法、产物是否完整也不在本地判定，轮询只检测文件**存在性** | 黄灯 + 错误提示 | 重新派发、手动标记完成、放弃任务 |
...
- 项目名称与当前状态
- 各任务按状态分组的计数（completed / running / pending / needs_attention / abandoned）
- 各任务明细：编号、名称、状态、指派 Agent、`result_file_path`、完成时间
- 人工介入与状态变更事件（来自 `task_events`）
