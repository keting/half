...
| `completed` / 已完成 | 检测到并校验通过 `result.json`，或负责人手动标记 | 查看结果、继续后续任务 |
| `needs_attention` / 需人工处理 | 超过任务超时时间仍未检测到 `result.json`，或 `result.json` 校验失败 | 根据 `last_error` 修正结果文件、重新派发、手动标记完成、放弃任务 |
