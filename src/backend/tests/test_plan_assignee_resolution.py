...
class PlanAssigneeResolutionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.db = MagicMock()
        self.db.query.return_value.all.return_value = self.agents

    def test_resolves_assignee_by_slug(self):
        self.assertEqual(_resolve_assignee_agent_id(self.db, "codex-plus"), 1)

    def test_resolves_assignee_by_display_name(self):
        self.assertEqual(_resolve_assignee_agent_id(self.db, "Claude Max"), 2)

    def test_resolves_assignee_by_agent_type(self):
        self.assertEqual(_resolve_assignee_agent_id(self.db, "chatgpt-plus"), 1)

    def test_returns_none_when_unmatched(self):
        self.assertIsNone(_resolve_assignee_agent_id(self.db, "unknown-agent"))
